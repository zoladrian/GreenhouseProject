...
- **Adres IEEE w JSON:** po zmianie *friendly name* w Z2M zmienia się **temat** MQTT. Aplikacja używa **`ieee_address` / `device.ieeeAddr`** jako stabilnego `ExternalId` i **scala** z istniejącym wpisem po starej nazwie z topicu (rekey + ta sama nazwa wyświetlana z aktualnego topicu). Przy pierwszym ingestcie po zmianie nazwy szuka też **IEEE w ostatnich odczytach** (do 50 wpisów), żeby połączyć stary rekord bez IEEE w najnowszym JSON. W `docker-compose` włączone jest **`mqtt.include_device_information`** w Z2M (wymagane do IEEE w payloadzie).
- Zakładka **Sensory** pokazuje zarejestrowane czujniki; **Przypisanie do nawy** powoduje, że wilgotność i wykresy pojawiają się na **Dashboardzie** i w szczegółach nawy. Wykres wilgotności: pionowe linie epizodów (podlanie vs heurystyka deszczu) — [**ANALYTICS_WATERING.md**](ANALYTICS_WATERING.md). **Więcej wpisów niż urządzeń w Z2M** — to zwykle stare rekordy w SQLite (inna nazwa / inny dongiel); scalanie przy starcie łączy tylko ten sam IEEE; nadmiar można usunąć przyciskiem **Usuń z listy aplikacji** na stronie Sensory (`DELETE /api/sensor/{id}` — odczyty zostają w bazie z odłączonym `SensorId`).
- **Stare duplikaty** (np. dwa rekordy: nazwa z topicu + ten sam układ po IEEE): przy **starcie API** uruchamiane jest jednorazowe **scalanie** (przeniesienie `SensorReadings`, scalenie przypisania do nawy, usunięcie zbędnego rekordu) na podstawie IEEE w historii odczytów. Nie trzeba czyścić bazy ręcznie.
- **Jedna seria danych na czujnik:** nowe odczyty zapisują w polu identyfikatora **kanoniczny `ExternalId`** (IEEE z JSON, a dopóki go nie ma — fragment topicu). Po **rekey** na IEEE i przy starcie API wykonywane jest **wyrównanie** starych wierszy odczytów do aktualnego `ExternalId`, żeby zmiana *friendly name* w Z2M nie rozbijała wykresów. **Wyświetlana nazwa** (`DisplayName`) nadal podąża za topicem Z2M — osobno od stabilnego klucza IEEE.
...
To **osobna** rzecz od Dockera: sieć AP (`hostapd` / hotspot) musi być włączona i skonfigurowana **na hoście** Raspberry Pi (np. *Raspberry Pi Connect* / ręczny AP). **Kontener `greenhouse-api` nie może** niezawodnie włączać Wi‑Fi (brak `wlan0` / NetworkManager w środku) — automatyczne **utrzymanie hotspotu** jest w repozytorium jako skrypt **systemd na hoście**: [**scripts/pi-hotspot/README.md**](../scripts/pi-hotspot/README.md). Captive portal: [**CAPTIVE-PORTAL.md**](CAPTIVE-PORTAL.md). Panel działa też po **LAN**: `http://<IP_malinki>:5000`.
